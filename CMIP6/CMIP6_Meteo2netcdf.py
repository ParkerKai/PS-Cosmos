...
dir_out1 = r'Y:\PS_Cosmos\02_models\DFM_Regional\CMIP6';
dir_out2 = r'Y:\PS_Cosmos\01_data\Climate\CMIP6\Meteo';
...
year_vec = [int(time_wnt[i].strftime("%Y")) for i in range(len(time_wnt))]
year_vec = np.array(year_vec)
month_vec = [int(time_wnt[i].strftime("%m")) for i in range(len(time_wnt))]
...
for yr in years:
    ...
    ind_wy = np.logical_and((time_wnt >= (datetime.datetime(yr-1, 10, 1, 0, 0)- datetime.timedelta(days=buff))), (time_wnt <= (datetime.datetime(yr, 9, 30, 0, 0)+datetime.timedelta(days=buff))))
    ...
    data_vars = {'u10':(['time','longitude','latitude'], uas_yr, 
                             {'units': 'm/s', 
                              'standard_name':'eastward_wind'}),
                 'v10':(['time','longitude','latitude'], vas_yr, 
                                          {'units': 'm/s', 
                                           'standard_name':'northward_wind'}),
                 'msl':(['time','longitude','latitude'], psl_yr, 
                                          {'units': 'Pa', 
    ...
    ds_out['longitude'].attrs = {"units": 'degrees_north',
                       'standard_name': 'longitude'}
    ds_out['latitude'].attrs = {"units": 'degrees_east',
                       'standard_name': 'latitude'}
    
    # Output the file  
    # ds_out.to_netcdf(os.path.join(dir_out,model,ver,
    #                               'psfm_{}_meteo_WY{}.nc'.format(mod_short,yr)),
    ...
    # Hack version to write to the DFM run folder. THe original folder is on the dead drive so direct writing.
    ds_out.to_netcdf(os.path.join(dir_out1,model,ver,'Meteo','WY_{}'.format(yr),
                                  'psfm_cmip6_meteo_WY{}.nc'.format(yr)),
                      encoding={'u10': {'dtype': 'float32', '_FillValue': -9999},
                                'v10': {'dtype': 'float32', '_FillValue': -9999},
                                'msl': {'dtype': 'float32', '_FillValue': -9999},
                                'time': {'dtype': 'int64', '_FillValue': -9999},
    ...
    ds_out.to_netcdf(os.path.join(dir_out2,model,ver, f'psfm_{mod_short}_meteo_WY{yr}.nc'),
                      encoding={'u10': {'dtype': 'float32', '_FillValue': -9999},
                                'v10': {'dtype': 'float32', '_FillValue': -9999},
                                'msl': {'dtype': 'float32', '_FillValue': -9999},
                                'time': {'dtype': 'int64', '_FillValue': -9999},
...
data_vars = {'u10':(['time','longitude','latitude'], uas, 
                         {'units': 'm/s', 
                          'standard_name':'eastward_wind'}),
             'v10':(['time','longitude','latitude'], vas, 
                         {'units': 'm/s', 
                          'standard_name':'eastward_wind'}),
             'msl':(['time','longitude','latitude'], psl, 
                             {'units': 'Pa', 
                              'standard_name':'air_pressure_fixed_height'})}
...
# Output the file  
ds_out.to_netcdf(os.path.join(dir_out2,model,ver,
                              'psfm_{}_meteo2.nc'.format(mod_short)),
                 encoding={'u10': {'dtype': 'float32', '_FillValue': -9999},
                           'v10': {'dtype': 'float32', '_FillValue': -9999},
                           'msl': {'dtype': 'float32', '_FillValue': -9999},
                           'time': {'dtype': 'int64', '_FillValue': -9999},
