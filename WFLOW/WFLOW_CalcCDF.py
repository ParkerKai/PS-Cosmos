...
def main():
    ...
    for Mod in Mod_list:
        ...
        ds_cmip = xr.open_mfdataset(
            files, engine="h5netcdf", parallel=True, chunks={"time": 1000}
        )

        # Loop months 1..12
        for month in range(1, 13):
            print(f"Processing Month: {month:02d}")
            cdf_month = monthly_CDF(ds_cmip, var, station_dim, month)[
                0
            ]  # [0] to unwrap dask.compute result
            ...
            out_path = os.path.join(
                out_dir,
                f"CDFmonthly_{month:02d}.{'parquet' if save_as_parquet else 'pkl'}",
            )
            if save_as_parquet:
                # Requires `pyarrow`
