...
#dir_in = r'D:\DFM'
dir_in = r'Y:\PS_Cosmos\02_models\WFLOW'
dir_out = r'Y:\PS_Cosmos\02_models\WFLOW'
...
def calc_diff(cdf_H,cdf_F,cdf_R,data_month,var):
    ...
    for stat in range(data_month.dims['Q_contour_gauges_contour']):
        ...
        cdf_R_stat_cdf = dask.delayed(cdf_R['cdf'].loc[cdf_R['stat'] == stat].to_numpy())
        cdf_R_stat_val = dask.delayed(cdf_R['values'].loc[cdf_R['stat'] == stat].to_numpy())
        
        vals_era5 = dask.delayed(data_month[var].isel(Q_contour_gauges_contour=stat).values)
...
if (cnty == 'pierce') or (cnty == 'king'): 
    files = os.path.join(dir_in,'20240801_discharges',cnty,'era5_3hourly','output_scalar.nc')
    
elif (cnty == 'pierce') or (cnty == 'king'): 
    files = os.path.join(dir_in,'11_20_2025_Discharges_SnohomishKitsap',cnty,'era5_3hourly','output_scalar.nc')
...
for cnt,Mod in enumerate(Mod_list):
    ...
    for month in np.arange(1, 13, 1, dtype=int):
        ...
        with open(os.path.join(dir_in,'20240801_discharges',cnty, f'cmip6_{Mod}_historic_bc',
                               'CDFmonthly_{0:02d}.pkl'.format(month)), 'rb') as f: 
            cdf_cmipH = pickle.load(f)

         # Load the CMIP6 future data
        with open(os.path.join(dir_in,'20240801_discharges',cnty, f'cmip6_{Mod}_future_bc',
                               'CDFmonthly_{0:02d}.pkl'.format(month)), 'rb') as f: 
            cdf_cmipF = pickle.load(f)
        
         # Load the ERA5 data
        with open(os.path.join(dir_in,'20240801_discharges',cnty,'era5_3hourly',
                               'CDFmonthly_{0:02d}.pkl'.format(month)), 'rb') as f: 
        ...
        diff, quants = calc_diff(cdf_cmipH,cdf_cmipF,cdf_cmipR,data_month,'Q_contour')    
...
ds_full =  xr.Dataset({
    ...
    'Q_quants':ds_quants,
    'cmip_diff':ds_diff},
    attrs = {'DataSource': 'Y:\WFLOW',
             'ProducedBy': 'Wflow team (Joost Buitink & Brendan Dalmijn) and Kai Parker',
             'General': 'Extracted from WFLOW model version 20240419'})

output_yearly(ds_full,os.path.join(dir_out,'20240801_discharges',cnty,'cdf_diff'),'WFLOW_ERA5Diff_{year}.nc')
