...
dir_in_cmip6_waves = r"Y:\PS_Cosmos\GIS\Waves\Wave_CmipDiff_byModel_archive"
...
gauge_IDs = ["9447130", "9449880"]
countys = ["King", "SanJuan"]
...
def LoadWaveLUTmats(file_in):
    ...
    with h5py.File(file_in, "r") as f:
        ...
        u16 = np.array(dfm_id, dtype="<u2")         # enforce little-endian
        u16 = u16.transpose()
        strings = [
            row.tobytes().decode("utf-16-le").rstrip("\x00")
            for row in u16
        ]
        dfm_id = np.array(strings, dtype="U40")  # array of <U40
...
# ===============================================================================
# %% Loop through the gauges
# ===============================================================================

fig, ax = matplotlib.pyplot.subplots(2, 2, figsize=[8, 7])

for gauge_cnt, gauge_ID in enumerate(gauge_IDs):
    county = countys[gauge_cnt]
    ...
    for cnt, Mod in enumerate(Mod_list_DFM):
    ...
    # Sort because the quantile change may have shifted the order
    Cmip6_WL_RIs = np.array(Cmip6_RIs, dtype=float)
    Cmip6_WL_RIs = np.sort(Cmip6_WL_RIs, axis=0)
    ...
    for cnt, Mod in enumerate(Mod_list_Wave):
        print(f"Processing: {Mod}")
    ...
    Cmip6_Wave_RIs = np.sort(Cmip6_Wave_RIs, axis=0)
    ...
    # ===============================================================================
    ax[gauge_cnt, 0].plot(
        RIs_WL.index, RIs_WL["RI_emp_Model"], "k", label="Reanalysis", linewidth=2
    )
    ax[gauge_cnt, 0].plot(
    ...
    ax[gauge_cnt, 0].plot(Cmip6_RPs, Cmip6_WL_RIs[:, 0], "--", label="CNRM")
    ax[gauge_cnt, 0].plot(Cmip6_RPs, Cmip6_WL_RIs[:, 1], "--", label="EC-Earth")
    ax[gauge_cnt, 0].plot(Cmip6_RPs, Cmip6_WL_RIs[:, 2], "--", label="GFDL")
    ax[gauge_cnt, 0].plot(Cmip6_RPs, Cmip6_WL_RIs[:, 3], "--", label="HadGEM_HH")
    ax[gauge_cnt, 0].plot(Cmip6_RPs, Cmip6_WL_RIs[:, 4], "--", label="HadGEM_HM")
    ax[gauge_cnt, 0].plot(Cmip6_RPs, Cmip6_WL_RIs[:, 5], "--", label="HadGEM_HMsst")
    ax[gauge_cnt, 0].plot(Cmip6_RPs, Cmip6_WL_RIs[:, 6], "--", label="CMCC")

    ax[gauge_cnt, 0].fill_between(
        Cmip6_RPs,
        Cmip6_WL_RIs_CIs[0],
        Cmip6_WL_RIs_CIs[1],
        color="k",
        alpha=0.2,
        label="Cmip6 Ensemble 95% CI",
    )

    ax[gauge_cnt, 0].set_xscale("log")
    ax[gauge_cnt, 0].set_xlim([1, 30])

    ax[gauge_cnt, 1].plot(
        RIs_Wave.index, np.sort(RIs_Wave["RI_emp"]), "k", label="Reanalysis", linewidth=2
    )
    ax[gauge_cnt, 1].plot(
        Cmip6_RPs,
        np.average(Cmip6_Wave_RIs, 1),
        "b",
        label="CMIP6 Average",
        linewidth=2,
    )

    ax[gauge_cnt, 1].plot(Cmip6_RPs, Cmip6_Wave_RIs[:, 0], "--", label="CNRM")
    ax[gauge_cnt, 1].plot(Cmip6_RPs, Cmip6_Wave_RIs[:, 1], "--", label="EC-Earth")
    ax[gauge_cnt, 1].plot(Cmip6_RPs, Cmip6_Wave_RIs[:, 2], "--", label="GFDL")
    ax[gauge_cnt, 1].plot(Cmip6_RPs, Cmip6_Wave_RIs[:, 3], "--", label="HadGEM_HH")
    ax[gauge_cnt, 1].plot(Cmip6_RPs, Cmip6_Wave_RIs[:, 4], "--", label="HadGEM_HM")
    ax[gauge_cnt, 1].plot(Cmip6_RPs, Cmip6_Wave_RIs[:, 5], "--", label="HadGEM_HMsst")
    ax[gauge_cnt, 1].plot(Cmip6_RPs, Cmip6_Wave_RIs[:, 6], "--", label="CMCC")

    ax[gauge_cnt, 1].fill_between(
        Cmip6_RPs,
    ...
    ax[gauge_cnt, 1].set_xscale("log")
    ax[gauge_cnt, 1].set_xlim([1, 30])
   
# ===============================================================================

ax[0, 0].set_title("Seattle Extreme Water levels")
ax[0, 0].set_ylabel("Water Level (m)")
ax[0, 0].set_xticklabels([])
ax[0, 0].grid(which="both")

ax[0, 1].set_title("Seattle Extreme Wave Heights")
ax[0, 1].set_ylabel("Significant Wave Height (m)")
ax[0, 1].legend()
ax[0, 1].set_xticklabels([])
ax[0, 1].grid(which="both")


ax[1, 0].set_title("Friday Harbor Extreme Water levels")
ax[1, 0].set_xlabel("Return Period (year)")
ax[1, 0].set_ylabel("Water Level (m)")
ax[1, 0].grid(which="both") 


ax[1, 1].set_title("Friday Harbor Extreme Wave Heights")
ax[1, 1].set_xlabel("Return Period (year)")
ax[1, 1].set_ylabel("Significant Wave Height (m)")
ax[1, 1].grid(which="both") 


ticks_major = [1, 5, 10, 20, 30]

for a in (ax[1, 0], ax[1, 1]):
    a.set_xscale('log')
    a.set_xlim(1, 30)

    # Make exactly these the major ticks (labels will be shown)
    a.xaxis.set_major_locator(mticker.FixedLocator(ticks_major))
    a.xaxis.set_major_formatter(mticker.FuncFormatter(lambda x, pos: f'{x:g}'))

    # Optional: minor ticks for gridlines only (no labels)
    a.xaxis.set_minor_locator(mticker.LogLocator(base=10, subs=np.arange(2, 10) * 0.1))
    a.xaxis.set_minor_formatter(mticker.NullFormatter())

    # Looks cleaner with lighter minor grid
    a.grid(which='major', axis='x', alpha=0.6)
    a.grid(which='minor', axis='x', alpha=0.3)
...
fig.savefig(
    os.path.join(dir_out, "ReturnIntervalCurves_SeattleFridayHarbor.tiff"), dpi=300
)
