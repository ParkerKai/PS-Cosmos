...
def main():
    ...
    # Load the data
    files = glob(os.path.join(dir_ERA5, "ERA5_cdf*"))
    ...
    ds_full = xr.open_mfdataset(
        ...
        combine="nested",
        concat_dim="time",
        preprocess=_preprocess,  # normalize function
        data_vars="minimal",
        coords="minimal",
        compat="override",
        chunks={"time": 52560, "station": 64},
    ...
    files = glob(os.path.join(dir_Tidal, "*.nc"))
    ds_tidal = xr.open_mfdataset(
        files,
        engine="netcdf4",
        parallel=True,
    ...
    files = glob(os.path.join(dir_diff, "*.nc"))
    ds_diff = xr.open_mfdataset(
        files,
        engine="netcdf4",
        parallel=True,
