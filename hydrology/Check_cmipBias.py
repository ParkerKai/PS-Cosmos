...
#huc2.to_file(os.path.join(dir_out,'Huc_buffer.shp'))
...
pr_q0,pr_q1,pr_q2,pr_q3 = Extract_quarterly(ds_era5_huc,time_pds)


fig, [ax1,ax2,ax3,ax4] = matplotlib.pyplot.subplots(nrows=4, ncols=1, figsize=(5, 8))
ax1.violinplot(pr_q0,positions = np.ones(1),showmeans=True)
ax2.violinplot(pr_q1,positions = np.ones(1),showmeans=True)
ax3.violinplot(pr_q2,positions = np.ones(1),showmeans=True)
ax4.violinplot(pr_q3,positions = np.ones(1),showmeans=True)
...
cmip_ave_qtr = np.empty([len(mod_short),4],dtype = 'float64')
for cnt,model in enumerate(mod_short):
    ...
    cmip_ave_qtr[cnt,:]= Average_quarterly(ds_cmip_huc,time_pds)
    pr_q0,pr_q1,pr_q2,pr_q3 = Extract_quarterly(ds_cmip_huc,time_pds)

    # 
    ax1.violinplot(pr_q0,positions = np.ones(1)+cnt+1,showmeans=True)
    ax2.violinplot(pr_q1,positions = np.ones(1)+cnt+1,showmeans=True)
    ax3.violinplot(pr_q2,positions = np.ones(1)+cnt+1,showmeans=True)
    ax4.violinplot(pr_q3,positions = np.ones(1)+cnt+1,showmeans=True)
...
ax4.set_xticklabels(['ERA5','CMCC','CNRM','ECEarth','GFDL','HadGemHH','HadGemHM','HadGemSST'], rotation = 45, ha="right")

if Var == 'pr':  # Precipitation
    ax3.set_ylabel('Precip. Flux (mm/hr)')
    fig.savefig(os.path.join(dir_out,'Precip_Compare_{}.png'.format(huc_name)),dpi = 400)
    
elif Var == 'temp': # Temperature
    ax3.set_ylabel('Temperature (celcius)')
    fig.savefig(os.path.join(dir_out,'Temp_Compare_{}.png'.format(huc_name)),dpi = 400)
