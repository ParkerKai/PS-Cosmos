...
for basin in np.unique(pull["BasinID"]):
    ...
    vp2 = axs.violinplot(
        data.loc[(data["BasinID"] == basin) & (data["slr"] == 1)][metric].dropna()[
            lambda x: (x > -2) & (x != 0)
        ]
        * 100,
        positions=[basin * 5 + cnt + 2],
    ...
    vp3 = axs.violinplot(
        data.loc[(data["BasinID"] == basin) & (data["slr"] == 2)][metric].dropna()[
            lambda x: (x > -2) & (x != 0)
        ]
        * 100,
        positions=[basin * 5 + cnt + 3],
    ...
    vp4 = axs.violinplot(
        data.loc[(data["BasinID"] == basin) & (data["slr"] == 3)][metric].dropna()[
            lambda x: (x > -2) & (x != 0)
        ]
        * 100,
        positions=[basin * 5 + cnt + 4],
    ...
    vp5 = axs.violinplot(
        data.loc[(data["BasinID"] == basin) & (data["slr"] == 4)][metric].dropna()[
            lambda x: (x > -2) & (x != 0)
        ]
        * 100,
        positions=[basin * 5 + cnt + 5],
...
axs.plot([-0.5, 70], [0, 0], color="black", linewidth=2, linestyle="--")
axs.set_title("Change in Significant Wave Height")
axs.grid()
axs.set_xlim([0, 69])
plt.xticks(
    ticks=[2.5, 9.5, 16.5, 23.5, 30.5, 37.5, 44.5, 51.5, 58.5, 65.5],
